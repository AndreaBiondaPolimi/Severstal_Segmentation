...
\begin{abstract}
   ...
   The goal of this challenge is not only to provide a good segmentation and calssification of defects, but also an accurate classification between defective and non-defective images, in order to reduce false positives, and so waste of money and resources.
...
\section{Introduction}
   The proposed solution takes in input an image of steel sheet that comes from high-frequency cameras and output the same-size segmented image, which contains the area of the defect(s) identified (if any) and the corresponding type. The types of defects that is possible to face are 
